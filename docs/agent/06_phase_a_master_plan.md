...
>
> ⚠️ **基准说明：** 本文档的 13 阶段划分和所有 IC 测试基于 `index.000985.SH`（中证全指）计算。
> 2026-06-09 基准已切换为 `index.801003.SW`（申万Ａ指）。引用本文档前，需确认 `BM_SYMBOL` 已改为 `801003` 并重跑验证 IC。
> 重跑完成后删除本段。
...
基于基准指数 `index.801003.SW` 收盘价最高/最低点划分。
